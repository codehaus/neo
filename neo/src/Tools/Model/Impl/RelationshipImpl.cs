...
{
	public class RelationshipImpl : IRelationship
	{
		//--------------------------------------------------------------------------------------
		...
		public RelationshipImpl(EntityImpl anEntity)
		{
		...
		public IEntity ForeignEntity
		{
			get { return ((ModelImpl)LocalEntity.Model).GetEntityForTable(foreignTableName); }
		}
		...
		public RelType Type
		{
			get
			{
				if(((EntityImpl)ForeignEntity).ColumnIsPrimaryKey(ForeignKey))
...
//					return (direction == "parent") ? RelDirection.Parent : RelDirection.Child;
				...
				if(((EntityImpl)LocalEntity).ColumnIsPrimaryKey(LocalKey))
		...
		public IRelationship InverseRelationship
		{
			get
			{
				foreach(RelationshipImpl r in ForeignEntity.Relationships)
				{
